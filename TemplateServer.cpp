...
bool MDKDLLAPI CTemplateServer::Bind(const char *ip, int port, bool udp)
{
	...
	if (udp)
	{
		...
		uv_udp_init((uv_loop_t*)m_loop, real_udp_socket);
	}
	else
	{
		real_tcp_socket = (uv_tcp_t*)malloc(sizeof uv_tcp_t);
		uv_tcp_init((uv_loop_t*)m_loop, real_tcp_socket);
	}
	...
	if (udp)
		uv_udp_bind(real_udp_socket, (const struct sockaddr*)&addr, 0);
	else
		uv_tcp_bind(real_tcp_socket, (const struct sockaddr*)&addr, 0);
